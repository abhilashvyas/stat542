...
user_ratings = pd.Series(data=np.nan, index=rmat.columns)  # Ensure Series matches Rmat.csv columns

# Map ratings from sliders to the correct columns in Rmat.csv
for movie_id, rating in ratings.items():
    column_name = f"m{movie_id}"  # Format MovieID to match column names in Rmat.csv
    if column_name in user_ratings.index and rating > 0:  # Check if the column exists
        user_ratings[column_name] = rating

# Debug: Check if user_ratings is populated correctly
st.write("Debug Info: User ratings (after mapping):")
st.write(user_ratings)

# Check if at least one movie is rated before calling myIBCF
if user_ratings.notna().sum() == 0:
    st.warning("Please rate at least one movie to get recommendations.")
else:
    # Call myIBCF with the user-provided ratings
