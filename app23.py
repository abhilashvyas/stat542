...
")
    ...
    movies = pd.DataFrame(data, columns=["movie_id", "title", "genres"])
    return movies
...
for _, row in sample_movies.iterrows():
    ratings[row["movie_id"]] = st.radio(
        f"{row['title']} ({row['genres']})",
        options=[1, 2, 3, 4, 5],
        index=2,
        key=f"rating-{row['movie_id']}",
    )
...
if st.button("Get Recommendations"):
    ...
    if recommendations.empty:
        ...
        for movie_id in recommendations.index:
            movie_title = movies[movies["movie_id"] == int(movie_id[1:])]["title"].values[0]
